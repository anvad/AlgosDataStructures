...
class Rope:
  ...
  def traverseTree_rec(self, cur_node, traverseType):
    if cur_node == None:
      return
    ...
    if leftChild != None:
      self.traverseTreeRec(leftChild, traverseType)
    if traverseType == "in":
      self.res.append(self.s[cur_node.key])
    if rightChild != None:
      self.traverseTreeRec(rightChild, traverseType)
...
def findi_rec(root, index):
  ...
  if index == (cur_index):
    ...
    return findi_rec(root.left, index)
  elif index > (cur_index):
    #print("going to right child")
    return findi_rec(root.right, index - cur_index - 1)
